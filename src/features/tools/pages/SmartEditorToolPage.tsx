...
export function SmartEditorToolPage({ slug }: SmartEditorToolPageProps) {
  ...
  const isReview = isReviewSlug(slug) && isReviewTool(config);
  ...
  return (
    <ToolPageShell title={config.label}>
      ...
      <Card>
        <CardContent className="space-y-4 p-6">
          <p className="text-sm text-muted-foreground">
            {isReview
              ? `الصق النص ثم اضغط «${config.detectLabel}» — راجع المقاطع المظللة واختر ما تريد إعادة صياغته.`
              : `الصق النص ثم اضغط «تشغيل» — هذه الصفحة مخصّصة لأداة ${config.label} فقط.`}
          </p>
          <Textarea
          ...
          />
          {isReview ? (
            <Button onClick={startReview} disabled={processing}>
              <config.icon className="size-4" />
              {config.detectLabel}
            </Button>
          ) : (
            <Button onClick={runOneShot} disabled={processing}>
              <config.icon className="size-4" />
              تشغيل {config.label}
            </Button>
          )}
      ...
      {isReview && showReviewPanel && isReviewTool(config) ? (
